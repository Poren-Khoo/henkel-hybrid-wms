...
export default function LocationsTable({ warehouseCode }) {
  ...
  const [formData, setFormData] = useState({
    wh: warehouseCode || 'WH01', // Pre-fill with warehouse code
    zone: '',
    code: '',
    type: 'Storage',
    capacity: '1000',
  ...
  // 1. GET LIVE DATA - AUTOMATIC FILTER: Only show bins for this warehouse
  const locations = useMemo(() => {
    // If no warehouse code provided, return empty (e.g., when creating new warehouse)
  ...
  const handleEdit = (location) => {
    ...
    setFormData({
      wh: location.wh || warehouseCode || 'WH01',
      zone: location.zone || '',
      code: location.code || '',
  ...
  const handleExport = () => {
    ...
    link.setAttribute('download', `locations_${warehouseCode}_${new Date().toISOString().split('T')[0]}.csv`)
  ...
  const handleClearFilters = () => {
    setFilters({ zone: '', type: '', temp: '', status: '' })
    setSearchTerm('')
  }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
           ...
           <Button 
              ...
              onClick={() => {
                setEditingLocation(null)
                setFormData(prev => ({ ...prev, wh: warehouseCode || 'WH01', code: '', zone: '', utilization: '0' }))
                setIsModalOpen(true)
              }}
           >
            ...
            {!warehouseCode ? (
              <TableRow>
                <TableCell colSpan={7} className="h-24 text-center text-slate-500 text-sm">
                  Please save the warehouse first to manage locations.
                </TableCell>
              </TableRow>
            ) : filtered.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="h-24 text-center text-slate-500 text-sm">
                  {searchTerm ? "No matching locations found." : `No locations found for warehouse ${warehouseCode}.`}
                </TableCell>
              </TableRow>
            ) : (
              filtered.map((loc) => (
