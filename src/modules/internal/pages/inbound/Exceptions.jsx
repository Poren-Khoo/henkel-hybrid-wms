...
export default function Exceptions() {
  ...
  const exceptions = useMemo(() => {
    ...
    return list.map(ex => ({
      id: ex.id || `EX-${Math.floor(Math.random()*1000)}`,
      type: ex.type || 'UNKNOWN',
      severity: ex.severity || 'WARNING',
      sourceRef: ex.source_ref || ex.ref || 'N/A', // PO-123 or TASK-456
      reportedBy: ex.reported_by || 'Operator',
      timestamp: ex.timestamp || 'Just now',
  ...
  const handleResolve = (action) => {
    ...
    const payload = {
      exception_id: selectedEx.id,
      action: action, // 'ACCEPT', 'REJECT', 'WRITE_OFF'
      note: resolutionNote,
      resolver: "Supervisor_Account", // TODO: Auth
      timestamp: Date.now()
    }
