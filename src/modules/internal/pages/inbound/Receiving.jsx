...
// HELPER: Identify Manufacturing Type
const isManufacturing = (type) => ['PROD_FG', 'PROD_SFG', 'SUB_RET'].includes(type)
...
export default function Receiving() {
  ...
  const handleSelectASN = (doc) => {
    ...
    setHeaderData(prev => ({
      ...prev,
      truck: isManufacturing(doc.type) ? '' : 'Simulated Truck', 
      dnNumber: doc.id,
      prodLine: isManufacturing(doc.type) ? (doc.supplier || 'LINE-01') : '', // If Mfg, Supplier field usually holds Line ID
      hasCoa: true,
  ...
  const handleSubmit = (force = false) => {
    // 1. Validation
    if (lines.length === 0) return alert("At least one line item is required.")
    
    // Dynamic Validation based on Type
    if (!isManufacturing(selectedDoc?.type)) {
        if (!headerData.truck) return alert("Truck Plate is required for Commercial Receipts.")
    } else {
        if (!headerData.prodLine) return alert("Production Line ID is required for Manufacturing Receipts.")
    }

    const invalidLine = lines.find(l => !l.code || !l.qty || Number(l.qty) <= 0 || !l.batch || !l.mfgDate)
    if (invalidLine) return alert("All lines must have Material, Batch, Mfg Date, and positive Quantity.")

    // 2. Tolerance Check
    if (!isManual && !force && selectedDoc) {
        const totalReceived = lines.reduce((sum, l) => sum + Number(l.qty), 0)
        const expected = selectedDoc.lines.reduce((sum, l) => sum + (Number(l.qty) || Number(l.qty_expected) || 0), 0)
        
        if (expected > 0 && totalReceived > expected * 1.1) {
            setOverReceiptWarning({
                received: totalReceived,
                expected: expected,
                diff: ((totalReceived - expected) / expected * 100).toFixed(1)
            })
            return 
        }
    }
    ...
    const payload = {
      ...
      type: selectedDoc ? selectedDoc.type : 'MANUAL',
      
      // Polymorphic Context
      context: isManufacturing(selectedDoc?.type) ? {
          line: headerData.prodLine,
          shift: headerData.shift
      } : {
          dock: headerData.dock,
          truck: headerData.truck
      },
              ...
              livePendingDocs.map(doc => (
                ...
                >
                  ...
                  <div className="text-xs text-slate-500 flex items-center gap-1.5 mb-2">
                    ...
                    {isManufacturing(doc.type) ? <Factory className="h-3 w-3 text-emerald-600" /> : <Truck className="h-3 w-3" />}
                    <span className="truncate max-w-[150px]">{doc.supplier}</span>
                  </div>
                  <div className="flex items-center gap-2">
                      <Badge variant="outline" className={`text-[9px] h-4 px-1.5 border rounded-sm ${isManufacturing(doc.type) ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-blue-50 text-blue-700 border-blue-100'}`}>
                        {doc.type}
                      </Badge>
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="px-6 py-4 border-b border-slate-100 bg-white flex justify-between items-center">
                ...
                <div className="flex gap-4">
                    ...
                    {isManufacturing(selectedDoc?.type) ? (
                        <>
